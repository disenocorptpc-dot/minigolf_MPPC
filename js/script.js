    ...
    window.showMapDetail = (item) => {
        ...
        if (storyContent && storySection) {
            ...
            if (pagControls) {
                ...
                pagControls.style.setProperty('display', 'none', 'important');
            }

            // Set Content (Injecting content manually instead of using home story pages)
            storyContent.innerHTML = `
                <div class="map-detail-content">
                    <h2 style="color:var(--color-wax-red); font-family:var(--font-heading); margin-bottom: 20px;">${item.title}</h2>
                    ${item.content}
                    <button onclick="returnToMap()" style="display:block; margin: 30px auto; padding: 10px 25px; cursor:pointer; background:var(--color-wax-red); color:white; border:2px solid var(--color-wood); border-radius:4px; font-size:1.5rem; font-family:var(--font-heading);">Volver al Mapa</button>
                    <br><br>
                </div>
            `;
